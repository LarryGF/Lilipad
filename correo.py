...
##funcion para añadir delays a lista y contar el total de enviados
def to_line_process():
    cont2 = emails_send
        ...
        for linea in file2:
            ...
            if struct_line[0] == month2 and struct_line[1] == day and int(struct_line[2][0:2]) >= 11 and int(
                ...
                try:
                    ...
                    pass
        datos.writerow([id, delay, final_status, number_of_retries, error_cause])
        file2.close()

##determinar el tipo de error,esto se podria mejorar,incluyendo un diccionario con cada tipo de error,para no tener tantos if
...
def main(file_route,date):
    ...
    datos.writerow(['email id', 'final delay', 'final status', 'number of retries', 'error cause'])
    ...
    for key,value in sorted(dic.items(),key=itemgetter(1),reverse=True):
        per.writerow([(key),value,round(value*100/len(first_errors_id_list),3)])
    csvdatos.close()
